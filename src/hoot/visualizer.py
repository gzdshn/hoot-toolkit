...
def visualize_videos(data_directory: str, output_directory: Optional[str], video_key: Optional[str]) -> None:
    ...
    for class_dir in class_directories:
        ...
        for video_path in tqdm(video_dirs, desc='loading annotations...'):
            ...
            video_data = load_video_from_file(str(video_path), None, video_anno_json, video_meta_json)
...
def visualize_video(video_data: Video, output_folder: Optional[Path]=None, save: Optional[bool]=False, with_mask: Optional[str]="multi") -> None:
    '''TODO: DOCUMENTATION'''

    for idx, frame in enumerate(video_data.frames):
        img_data = cv2.imread(str(frame.frame_path)) ## TODO: record frame path in annotations, will depend on local path
        ...
        ## Resize cause images are BIG
        h,w = img_data.shape[:2]
        if h>1000:
            vis_data = cv2.resize(vis_data,(int(w*0.5),int(h*0.5)), interpolation = cv2.INTER_AREA)
        
        if output_folder and save:
            file_name = frame.frame_path.name
